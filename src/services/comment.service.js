...
export const replyComment = async ({ commentId, userId, text }) => {
	...
	try {
		...
		let commentReply = await new commentModel({
			comment: commentId,
			user: userId,
			text,
		});
		...
		return { message: "comment submitted successfully", data: commentReply };
	} catch (error) {
		throw new ErrorAndStatus(
			error.message || "An error occured, please try again later!",
			error.status || 500
...
export const allCommentReplies = async ({ commentId, userId, page }) => {
	if (!commentId) throw new ErrorAndStatus("post Id is required", 400);
	...
	try {
		...
		replies = await Promise.all(
			replies.map(async (reply) => {
				const replyLikes = await commentLikeModel
					.find({ comment: com._id })
					.lean();
...
export const deleteCommentReply = async ({ replyId, user }) => {
	...
	try {
		...
		await commentReplyModel.findByIdAndDelete(commentId);
		// await commentLikeModel.deleteMany({ comment: commentId });
		return true;
	} catch (error) {
		throw new ErrorAndStatus(
